...
\newpage
% ============================================================
\section{Resume section points (High-Density 3-Bullet ATS Format)}

The following 3 bullet points are formatted according to the high-density \textbf{What $\rightarrow$ How $\rightarrow$ Impact} framework matching the rest of the resume, preserving 100\% of technical and empirical metrics:

\vspace{0.3cm}
\noindent\textbf{Autonomous Relational Analytics Agent} $|$ \emph{Python, Pandas, Streamlit, Groq API, AST Sandbox, BFS, Plotly} \hfill SEP 2026

\begin{itemize}[leftmargin=0.6cm,itemsep=3pt]
\item \textbf{Architected an autonomous multi-agent analytics engine} with specialized \textbf{profiling, planning, coding, and storytelling agents} in a ChatGPT-style Streamlit UI with Plotly charts, collapsible AST execution DAGs, and JSONL query audit logging; built a dual-tier model failover ladder (\texttt{gpt-oss-120b} $\rightarrow$ \texttt{qwen3.8-27b} on HTTP 429 rate limits) backed by an offline heuristic compiler for zero-downtime, zero-API-key resilience.
\item \textbf{Engineered autonomous schema discovery and multi-hop join pathfinding} using \textbf{Breadth-First Search (BFS)} across candidate foreign keys to resolve non-adjacent table joins across star/snowflake schemas (up to 5 tables); dynamically synthesized cross-table algebraic metrics ($\text{quantity} \times \text{unit\_price} \times (1 - \text{discount})$) and temporal period buckets (\texttt{dt.to\_period('M')}) with zero hardcoded column or table hints.
\item \textbf{Built an AST execution sandbox with a 21-token injection blocklist} (\texttt{os}, \texttt{subprocess}, \texttt{drop table}), safe built-in scoping, a 1,000-row cap, and an automated 3-iteration self-healing repair loop (\texttt{MAX\_FIXES=3}); benchmarked across 5 suites on 15,000-row datasets with $\Delta \le 0.05$ tolerance, achieving \textbf{100\% pass rate (22/22 tests)} and \textbf{12/12 vs.\ 0/12 monthly revenue accuracy vs.\ naive ChatGPT}, eliminating a \textbf{7.0\% (Rs.~6.96L) financial error margin} caused by hallucinated order filters.
\end{itemize}
