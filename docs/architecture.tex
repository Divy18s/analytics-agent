...
\item \textbf{Engineered autonomous schema discovery and multi-hop join pathfinding} using \textbf{Breadth-First Search (BFS)} across foreign keys to bridge non-adjacent tables in star/snowflake schemas (up to 5 tables); dynamically synthesized cross-table formulas ($\text{qty} \times \text{price} \times (1 - \text{discount})$) and monthly period buckets with zero user join hints.
