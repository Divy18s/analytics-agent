...
\newpage
% ============================================================
\section{Resume section points (Domain-Agnostic 3-Line High-Density Format)}
...
\item \textbf{Architected a domain-agnostic multi-agent analytics engine} with specialized \textbf{profiling, planning, coding, and storytelling agents} in a Streamlit UI with Plotly charts and execution traces; implemented dual-model failover (\texttt{gpt-oss-120b} $\rightarrow$ \texttt{qwen3.8-27b} on 429s) and an offline heuristic compiler for zero-downtime resilience.
\item \textbf{Engineered autonomous schema discovery and multi-hop join pathfinding} using \textbf{Breadth-First Search (BFS)} across foreign keys to bridge non-adjacent tables in complex star/snowflake schemas; dynamically synthesized arbitrary multi-column arithmetic expressions, filter predicates, and temporal period buckets with zero join hints.
\item \textbf{Built an AST execution sandbox with a 21-token injection blocklist} (\texttt{os}, \texttt{subprocess}, \texttt{drop table}), safe built-ins, 1,000-row cap, and an automated 3-iteration self-healing loop; achieved \textbf{100\% pass rate (22/22 tests)} across 5 multi-domain suites on 15k rows, eliminating naive ChatGPT's silent row drops and filter hallucinations (which caused a 7.0\% error).
\end{itemize}
