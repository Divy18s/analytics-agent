...
\newpage
% ============================================================
\section{Resume section points (ATS-friendly)}

The following 4 bullet points are formatted according to the \textbf{What $\rightarrow$ How $\rightarrow$ Impact} framework, optimized for ATS parsers and technical interview discussions:

\vspace{0.3cm}
\noindent\textbf{Autonomous Relational Analytics Agent} $|$ \emph{Python, Pandas, Streamlit, Groq API, AST Sandbox, BFS} \textbf{(\href{https://github.com/Divy18s/analytics-agent}{GITHUB})} \hfill Sept 2026

\begin{itemize}[leftmargin=0.6cm,itemsep=3pt]
\item \textbf{Architected a multi-agent analytics pipeline} with specialized \textbf{profiling, planning, coding, and storytelling agents} that converts natural language business questions into executable Pandas workflows without SQL or schema hints.
\item \textbf{Engineered autonomous multi-table join discovery} using \textbf{Breadth-First Search (BFS) pathfinding} across foreign key candidates to resolve multi-hop joins across non-adjacent tables in complex star/snowflake schemas.
\item \textbf{Built a sandboxed execution and self-healing loop} featuring a \textbf{21-token AST security blocklist}, safe built-in whitelisting, and an automated 3-iteration exception repair cycle (\texttt{MAX\_FIXES=3}) under a strict 1,000-row cap.
\item \textbf{Benchmarked against naive ChatGPT on 5-CSV schemas}, achieving \textbf{12/12 vs.\ 0/12 exact monthly revenue matches} and a \textbf{100\% pass rate (22/22 tests)}, eliminating a \textbf{7.0\% (Rs.~6.96L) financial error margin} caused by ChatGPT's hallucinated order filters.
\end{itemize}
